...
You specify mnemonics by prefixing the letter intended to be the mnemonic with an ampersand (<![CDATA[&]]>) in the label definition.
For example, the label <![CDATA[&Save]]> with the S underlined (<emphasis role="underline">S</emphasis>) when the kbd:[Alt] key is pressed.
