...
=== Configure API baseline
		...
		TIP:
			Handling baseline issues and API is something which the core
			...
			For this
			select the
			menu:Window[Preferences > Plug-in Development > API Baselines]
			menu entry.
		...
		For example if you develop for the next Eclipse 4.x+1 release
			(for example 4.6),
			you should select
			the official
			Eclipse release
...
image::apibaseline20.png[Setting up the API baseline,pdfwidth=60%]
			...
			and
			https://wiki.eclipse.org/PDE/API_Tools/User_Guide">API Tools
			.
