...
The native widgets of the OS are accessed by the SWT framework via the
_Java Native Interface_
(JNI)
framework. JNI is a
programming
framework
...
Each `Shell` is constructed with a `Display` and if none is provided during construction it will use either the `Display` which is currently used or a default one.
