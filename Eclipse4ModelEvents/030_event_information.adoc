...
image::./images/event_tags.png[EventTags interface with properties of an Event,pdfwidth=60%]
...
image::./images/label_attname.png[Label ATTNAME,pdfwidth=60%]
...
For an example see <xref linkend="active_sample_event" />
...
image::./images/event_types.png[EventTypes interface for changes to the application model,pdfwidth=60%]
...
image::./images/event_type_convenience.png[Convenience methods for event types.,pdfwidth=60%]

The `isADD(Event)` and `isREMOVE(Event)` methods apply for both single and many, e.g., ADD and ADD_MANY)

			For an example see <xref linkend="position_event_tag" />
...
image::label_attname.png[Label ATTNAME,pdfwidth=60%]
