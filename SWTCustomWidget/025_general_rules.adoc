...
<xi:include xmlns:xi="http://www.w3.org/2001/XInclude" parse="text" href="./examples/CustomWidget.java"/>
</programlisting>

=== Tell others how big you are
By overriding the `computeSize` method you give a `Layout` and
other widgets around your custom widget the chance to get to know the size of your
custom widget.
By offering the size the `Layout`, of the parent `Composite`,
will be able to align its child widgets properly.
...
The <parameter>code</parameter> parameter of each `error` method must be one of
